...
def zero_list(len_list: int) -> list[float]:
    return [0 for _ in range(len_list)]


def zscore(x: npt.NDArray[float]) -> tuple[npt.NDArray[float], float, float]:
    """
    ...
    x_avg: float = np.mean(x)
    x_std: float = np.std(x, ddof=1)
    x_z: npt.NDArray = (x - x_avg) / x_std

    return x_z, x_avg, x_std


def inv_zscore(x_z: npt.NDArray, x_avg: float, x_std: float) -> npt.NDArray[float]:
    """
    ...
    """
    x: npt.NDArray = x_std * x_z + x_avg

    return x


def remove_infinite_weights(cme_par_dict: CmeParEns) -> CmeParEns:
    """
    Function to remove infinite weights from CME parameter dictionary
    :param cme_par_dict:  Dictionary that contains all CME parameters required
    :return: cme_par_dict:  Dictionary that contains all CME parameters
        required with infinite weights set to zero
    """
    assert "weight" in cme_par_dict.keys()

    # Extract weights of particles
    weights: npt.NDArray[float] = cme_par_dict["weight"]

    # If there are non-finite weights, set weight to zero (i.e. discard particle)
    weightCond: list[bool] = [~np.isfinite(w) for w in weights]
    if np.sum(weightCond) > 0:
        weights[weightCond] = 0

    cme_par_dict["weight"] = weights

    return cme_par_dict


def dict_keys_to_state_vector(
    ...
    """
    Function to place values from the cme parameter dictionary into a state vector array
    :param cme_par_dict: Dictionary that contains all CME parameters required
    :param state_ens: State vector ensemble matrix that will contain the parameters required for DA
    :param index_of_state: Index of the state vector that cme parameter 'par_name' is to be placed in
    :param par_name: Name of cme_parameter key that will be extracted
    :return: x_ens: State vector ensemble column for 'par_name' ensemble added in state_ens[:, index_of_state]
    """
    assert all(p in required_dict_keys() for p in par_names)
...
def add_units_from_cme_par(
    ...
    """
    Routine to add units from the CME parameters
    :param cme_par_array: CME parameters in an (nEns, nPar) array
    :param cme_par_dict: Dictionary containing CME parameters with following keys:
        ['t_init', 'v', 'width', 'lon', 'lat', 'thick', 'weight']
    :param par_names: List of names of parameters to add units to
    :return: CME parameter dictionary in appropriate units with astropy units added
    """

    # Initialise default par_names values and assert that they must match the required directory fields
    if par_names is None:
        par_names = ["t_init", "v", "width", "lon", "lat", "thick"]

    assert all(p in required_dict_keys() for p in par_names)

    nPar, nEns = np.shape(cme_par_array)
    ...
    cme_par_dict["v"]: Quantity[u.km / u.s] = cme_par_array[:, 1] * u.km / u.s
    cme_par_dict["width"]: Quantity[u.deg] = cme_par_array[:, 2] * u.deg

    cme_par_dict["lon"]: Quantity[u.deg] = cme_par_array[:, 3] * u.deg
    lonCond: list[bool] = cme_par_dict["lon"] < 0 * u.deg
    cme_par_dict["lon"]: Quantity[u.deg] = cme_par_dict["lon"][lonCond] + 360 * u.deg

    cme_par_dict["lat"]: Quantity[u.deg] = cme_par_array[:, 4] * u.deg
    cme_par_dict["thick"]: Quantity[u.solRad] = cme_par_array[:, 5] * u.solRad

    return cme_par_dict


def remove_units_from_cme_par(cme_par_dict: CmeParEns) -> npt.NDArray[float]:
    """
    Routine to remove units from the CME parameters
    :param cme_par_dict: Dictionary containing CME parameters with following keys:
        ['t_init', 'v', 'width', 'lon', 'lat', 'thick', 'weight']
        :param par_names: List of parameter names that units should be removed from
    :return: CME parameters in appropriate units but with astropy units removed
    """

    # Initialise default par_names values and assert that they must match the required directory fields
    print(cme_par_dict.keys())
    assert all(p in cme_par_dict.keys() for p in required_dict_keys())

    #nPar: int = len(par_names)
    ...
    #  cme_par_array[:, 4] = CME Latitude
    #  cme_par_array[:, 5] = CME thickness
    cme_par_array: npt.NDArray = np.zeros((nEns, 6))
    ...
    cme_par_array[:, 1]: Quantity[None] = cme_par_dict["v"].to(u.km / u.s).value
    cme_par_array[:, 2]: Quantity[None] = cme_par_dict["width"].to(u.deg).value

    cme_par_array[:, 3]: Quantity[None] = cme_par_dict["lon"].to(u.deg).value
    lonCond: list[bool] = list(cme_par_array[:, 3] > 180)
    cme_par_array[lonCond, 3]: Quantity[None] = cme_par_array[lonCond, 3] - 360

    cme_par_array[:, 4]: Quantity[None] = cme_par_dict["lat"].to(u.deg).value
    cme_par_array[:, 5]: Quantity[None] = cme_par_dict["thick"].to(u.solRad).value

    return cme_par_array


def cme_par_to_z_state_vector(
        cme_par_dict: CmeParEns, par_req: list[str]
    ...
    """
    assert all(p in required_dict_keys() for p in par_req)

    # Extract the ensemble of required parameters
    nPar: int = len(par_req)

    # Extract weights of particles
    cme_par_dict = remove_infinite_weights(cme_par_dict)

    # Extract number of ensemble members from length of weights array and initialise variables
    nEns: int = len(weights)

    state_ens: npt.NDArray[float] = np.zeros((nEns, nPar))
    z_ens: npt.NDArray[float] = np.zeros((nEns, nPar))
    x_mean: npt.NDArray[float] = np.zeros(nPar)
    x_std: npt.NDArray[float] = np.zeros(nPar)
    ...
    for i, ip in enumerate(par_req):
        par_temp: npt.NDArray[float] = cme_par_dict[ip]

        if ip == "lon":
            lonCond: list[bool] = par_temp > 180
            par_temp[lonCond] = par_temp[lonCond] - 360

        state_ens[:, i] = cme_par_dict[ip]
...
def z_state_vector_to_cme_par(
        z_ens: npt.NDArray[float],
        x_mean: npt.NDArray[float],
        x_std: npt.NDArray[float],
    ...
    # Put weights of particles into cme_par_dict
    cme_par_dict["weight"]: Quantity[None] = weights

    # Extract all required parameters, put them in an ensemble matrix, then calculate the zscores for output
    for i, ip in enumerate(par_req):
        par_temp: npt.NDArray[float] = inv_zscore(z_ens[:, i], x_mean[i], x_std[i])

        if ip == "lon":
            lonCond: list[bool] = par_temp < 0
            par_temp[lonCond]: npt.NDArray[float] = par_temp[lonCond] + 360

        cme_par_dict[ip] = par_temp

    return cme_par_dict



def cme_par_to_state_vector(cme_par_array: npt.NDArray, par_req: list[str]):
    """
    Function to convert the CME parameters into a state vector
    :param cme_par_array Array that contains all CME parameters required
    :param par_req List containing the names of all the required parameters from cme_par_dict
    :return state_ens Ensemble of state vectors
    """
    assert all(p in required_dict_keys() for p in par_req)
    ...
    for i, ip in enumerate(par_req):
        ...
        state_ens[:, i]: Quantity[None] = cme_par_array[:, indReq]

    return state_ens


def get_particle_weights(cme_par_dict):
    ## Function to convert the CME parameters into a state vector
...
def state_vector_to_cme_par(state_ens, weights, par_req, cme_par_dict):
    ## Function to convert the state vector back into the required CME parameters
    #  @param z_ens (nEns, nPar)-array containing the state ensemble converted to z-scores
    #  @param x_mean (nPar)-array containing the mean of the ensemble parameters
    #  @param x_std (nPar)-array containing the standard deviation of the ensemble parameters
...
def make_synthetic_obs(
    ...
    cme_width = true_cme_par_dict["width"].to(u.deg)
    cme_lon = true_cme_par_dict["lon"].to(u.deg)
    if cme_lon > (180 * u.deg):
        cme_lon = cme_lon - (360 * u.deg)

    cme_lat = true_cme_par_dict["lat"].to(u.deg)
...
def auxPf(
    ...
    return resampPar, resampWeights, cme_par_dict
...
def generateCovInitCMEPar(file_path, vars_req):
    ...
    dfCMEpar.drop(
        columns=[col for col in dfCMEpar if col not in vars_req], inplace=True
    )
...
def test_cme_par_to_state(nEns, seed=np.nan):
    ...
    init_par_arr["weight"] = [
        init_par_arr["weight"][i] / sum_weight for i in range(nEns)
    ]
    init_par_arr["likelihood"] = [
        init_par_arr["likelihood"][i] / sum_lik for i in range(nEns)
    ]
    ...
    final_par_arr = state_vector_to_cme_par(
        z_ens, x_mean, x_std, weights, par_req, final_par_arr
    )
...
def main():
    ...
    file_path = os.path.join(
        ...
        "PycharmProjects",
        "SIR_HUXt",
        "blairCMElistSingle.csv",
    )
    vars_req = ["v", "width", "lon", "lat", "thick"]
    ...
    true_cme_par_dict["t_init"] = true_cme_t_init
    true_t_init_str = true_cme_par_dict["t_init"].strftime("%Y%m%d-%H%M")

    true_cme_par_dict["v"] = true_cme_speed * u.km / u.s
    true_cme_par_dict["width"] = true_cme_width * u.deg
    true_cme_par_dict["lon"] = true_cme_lon * u.deg
    true_cme_par_dict["lat"] = true_cme_lat * u.deg
    true_cme_par_dict["thick"] = true_cme_thick * u.solRad
...
#        datetime.datetime(2008, 1, 1, 1, 0, 0),
    ...
    """cme_par_dict['v'] = np.array([500, 600, 400, 550]) * u.km / u.s
    print(np.mean(cme_par_dict['v']))
    cme_par_dict['width'] = np.array([60, 60, 60, 60]) * u.deg
    cme_par_dict['lon'] = np.array([5, 5, 5, 5]) * u.deg
    cme_par_dict['lat'] = np.array([5, 5, 5, 5]) * u.deg
    cme_par_dict['thick'] = np.array([5, 5, 5, 5]) * u.solRad"""
    # pars_req = ['v', 'lon', 'width']
    # pars, weights = cme_par_to_state_vector(cme_par_dict, pars_req)
    # obs = [23]
    ...
    fg_cme_t_init = datetime.datetime(2008, 1, 1, 1, 0, 0)
    fg_cme_t_init_str = fg_cme_t_init.strftime("%Y%m%d-%H%M")
    fg_cme_speed = 495
    fg_cme_width = 37.4
    fg_cme_lon = 0
    fg_cme_lat = 0
    ...
    sd_cme_t_init = 0  # In seconds
    sd_cme_speed = 50  # In km/s
    sd_cme_width = 5  # In deg
    sd_cme_lon = 5  # In deg
    sd_cme_lat = 0  # In deg
    sd_cme_thick = 0  # In solRad

    n_runs = 1

    outputFgDir = os.path.join(
        outBaseDir,
        f"truth_{true_t_init_str}_{true_cme_speed}_{true_cme_width}_{true_cme_lon}_{true_cme_lat}_{true_cme_thick}",
        f"prior_{fg_cme_t_init_str}_{fg_cme_speed}_{fg_cme_width}_{fg_cme_lon}_{fg_cme_lat}_{fg_cme_thick}",
        f"nEns-{n_ens}_{nObs}_{obs_lon}_{obs_lat}",
    )
    if not os.path.isdir(outputFgDir):
        os.makedirs(outputFgDir)

    for runNo in range(n_runs):
        outputDir = os.path.join(
            outputFgDir, f"runNo_{runNo:04d}"
        )
        ...
        #######################################################################
        # Initialise CME parameters
        mean_cme_t_init = fg_cme_t_init + datetime.timedelta(
            seconds=sd_cme_t_init * np.random.uniform(low=-1, high=1)
        )
        mean_cme_t_init_str = mean_cme_t_init.strftime("%Y%m%d-%H%M")
        mean_cme_speed = fg_cme_speed + (
                sd_cme_speed * np.random.uniform(low=-1, high=1)
        )
        mean_cme_width = fg_cme_width + (
                sd_cme_width * np.random.uniform(low=-1, high=1)
        )
        mean_cme_lon = fg_cme_lon + (sd_cme_lon * np.random.uniform(low=-1, high=1))
        mean_cme_lat = fg_cme_lat + (sd_cme_lat * np.random.uniform(low=-1, high=1))
        mean_cme_thick = fg_cme_thick + (
                sd_cme_thick * np.random.uniform(low=-1, high=1)
        )

        cme_par_dict["t_init"] = np.array([mean_cme_t_init for _ in range(n_ens)])
        cme_par_dict["v"] = (
                np.array(
                    [
                        np.random.uniform(
                            low=0.9 * mean_cme_speed, high=1.1 * mean_cme_speed
                        )
        ...
        cme_par_dict["width"] = (
                np.array(
                    [
                        np.random.uniform(low=mean_cme_width - 5, high=mean_cme_width + 5)
                        for _ in range(n_ens)
        ...
        cme_par_dict["lon"] = (
                np.array(
                    [
                        np.random.uniform(low=mean_cme_lon - 5, high=mean_cme_lon + 5)
                        for _ in range(n_ens)
        ...
        # cme_par_dict['lon'] = np.array(
        #     [mean_cme_lon
        #      for _ in range(n_ens)]
        # ) * u.deg
        cme_par_dict["lat"] = np.array([mean_cme_lat for _ in range(n_ens)]) * u.deg
        cme_par_dict["thick"] = (
                np.array([mean_cme_thick for _ in range(n_ens)]) * u.solRad
        )
        ...
        #print(np.cov(cme_saved_pars[0, :, :], axis=1))
        ###############################################################################
        ...
        for yi, y_obs in enumerate(synth_obs):
            ...
            inflationFact = 1#inflTerm1 + inflTerm2
            # -(7.0 * y_obs / 15.0) + 15.0

            inflExp = -(yi / 5.0) + 6
            # inflationFact = 2 ** inflExp
            print(f"inflation factor: {inflationFact}")
            resampPar, resampWeights, cme_par_dict = auxPf(
                cme_par_dict,
                y_obs,
                obs_cov,
                obs_lon,
            ...
            cme_saved_pars[yi + 1, lonCond, 3] = (
                    cme_saved_pars[yi + 1, lonCond, 3] - 360
            )

            cme_saved_pars[yi + 1, :, 4] = cme_par_dict["lat"].to(u.deg).value
            cme_saved_pars[yi + 1, :, 5] = cme_par_dict["thick"].to(u.solRad).value

            cme_v_values[:, yi] = cme_par_dict["v"].to(u.km / u.s).value
            cme_width_values[:, yi] = cme_par_dict["width"].to(u.deg).value
            # print(cme_par_dict)
